c *This file contains cuts on the pt of ll : line406-
...
C
     ...
     $         abs(ipdg(i)).eq.15) then
               if (abs(ipdg(j)).eq.12.or.abs(ipdg(j)).eq.14.or.
     $            abs(ipdg(j)).eq.16) then
                  tmpvar = invm2_04(p(0,i),p(0,j),1d0)
                  if (tmpvar.lt.2500**2) then
                     passcuts_user=.false.
                     return
                  endif
                  if (tmpvar.gt.4000**2) then
                     passcuts_user=.false.
                     return
                  endif
               endif
