...
C          Note that PDG specific cut are more optimised than simple user cut
      double precision (htmin=50)
      double precision (htmax=200)
      double precision (htevent=0)

      do i=1,nexternal   ! loop over all external particles
         if (istatus(i).eq.1 .and.
     &        (abs(ipdg(i)).le.maxjetflavor .or. ipdg(i).eq.21)) then
            htevent=htevent + sqrt(p(1,i)**2+p(2,i)**2)
         endif
         if (htevent .lt. htmin) then 
            passcuts_user=.false.
            return
         endif
         if (htevent .gt. htmax) then
            passcuts_user=.false.
            return
         endif
      enddo
